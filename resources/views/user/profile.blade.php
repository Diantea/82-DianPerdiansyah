                        ...
                        <form action="{{ $item ? route('user.update', $item) : route('user.store')}}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                ...
                                @if(in_array($item->role, ['student']))
                                    <div class="col-12">
                                        @include('forms.textfield', ['label' => 'NISN', 'field' => 'nisn', 'type' => 'text'])
                                    </div>
                                @endif

                                @if(in_array($item->role, ['student', 'teacher']))
                                    <div class="col-12">
                                        @include('forms.selectbox', ['label' => 'Jurusan', 'is_disabled' => true, 'field' => 'major_id', 'choices' => \App\Models\Major::orderBy('name', 'asc')->get()->map(function($major){ return ['label' => $major->name, 'value' => $major->id]; })])
                                    </div>
                                @endif

                                @if(in_array($item->role, ['student']))
                                    <div class="col-12">
                                        @include('forms.textfield', ['label' => 'Kelas', 'is_disabled' => true, 'field' => 'class', 'type' => 'number'])
                                    </div>
                                @endif
