            ...
            <div class="col-lg-8 col-md-8">
                <div class="card">
                    <div class="card-body">
                        ...
                        @php
                            $users = [
                                ['label' => 'Siswa', 'field' => 'student'],
                                ['label' => 'Guru Pembimbing', 'field' => 'teacher'],
                                    ...
                                    <div class="tab-pane fade show {{ (request()->get('role') ? (request()->get('role') === $user['field']) : ($i === 0)) ? 'active' : '' }}" id="navs-{{ $i }}" role="tabpanel">
                                        ...
                                        <form action="{{ $item ? route('user.update', $item) : route('user.store')}}" method="POST" enctype="multipart/form-data">
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="col-6">
                                                    @include('forms.selectbox', ['label' => 'Jurusan', 'field' => 'major_id', 'choices' => \App\Models\Major::orderBy('name', 'asc')->get()->map(function($major){ return ['label' => $major->name, 'value' => $major->id]; })])
                                                </div>
                                                @endif

                                                @if(in_array($user['field'], ['student']))
                                                <div class="col-6">
                                                    @include('forms.textfield', ['label' => 'Kelas', 'field' => 'class', 'type' => 'number'])
                                                </div>
                                                @endif
